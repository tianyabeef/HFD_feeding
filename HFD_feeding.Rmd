...
```{r,cache=TRUE}
# profiling_species_absolute <- read.table("wucy/lipidmice.species", sep = "\t",header=T, row.names=1, dec=".")
# profiling_species <- read.table("wucy/uniform_lipidmice.species",sep="\t",header=T,row.names=1,dec=".")
# profile_phylum <- read.table("wucy/uniform_lipidmice.phylum",sep="\t",header=T,row.names = 1,dec=".")
# profile_genus <- read.table("wucy/uniform_lipidmice.genus",sep="\t",header=T,row.names = 1,dec=".")
###Load the necessary data.
load("data/fig3a-abc.RData")
...
```{r,cache=TRUE,fig.height=6,width=8}
par(opar)
data <- profiling_species_absolute
###Abundance of species filter
data[data <= 1e-6] <- 0
